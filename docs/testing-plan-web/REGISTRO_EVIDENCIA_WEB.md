...
**Estado de Fase 4:** empleabilidad y postulación ejecutadas; se conserva la evidencia de Fases 1–3.
...
El `include` de cobertura comprende `src/App.tsx`, `src/services/**/*.ts`, los módulos de autenticación, administración, catálogo, contacto, empleabilidad y CMS, las páginas/hooks públicos priorizados y los helpers compartidos listados en `vite.config.ts`. El `exclude` omite `src/test/**`, tests, declaraciones, assets y primitivas `src/components/ui/**`. La cobertura de Fase 4 sube a 66.10% en líneas (669/1012), con 64.71% de statements (717/1108); continúa como métrica informativa y todavía no se aplica el umbral de 80% reservado para la Fase 9.

### Archivos críticos no cubiertos

| Archivo | Líneas/decisiones | Riesgo | Caso pendiente | Acción |
|---|---|---|---|---|
| `src/App.tsx`, `src/services/**` y módulos críticos incluidos | Cobertura global menor al 80% después de Fases 2–4 | CMS, UI transversal y E2E aún no tienen casos dirigidos; quedan ramas residuales en páginas públicas | Implementar casos WEB-CMS, WEB-UI y WEB-E2E; revisar líneas restantes | Añadir suites por riesgo en Fases 5–8 y activar el gate al completar el alcance crítico |
